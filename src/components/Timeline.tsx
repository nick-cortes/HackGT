...
export default function Timeline() {
  ...
  const startDate = new Date(Math.min(...publications.map(p => new Date(p.date))));
  const endDate = new Date(Math.max(...publications.map(p => new Date(p.date))));

  const computeLeft = (dateStr) => {
    const date = new Date(dateStr);
    const ratio = (date - startDate) / (endDate - startDate);
      ...
      >
        ...
        {publications.map(pub => (
          ...
          >
            ...
            <span className="text-sm text-white-700 mt-1">{pub.title}</span>
          </div>
        ))}
      </div>
    </div>
